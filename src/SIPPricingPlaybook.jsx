...
const DEFAULT_COST_DB = {
  ...
  sipDidBundles: {
    ...
    sipDidBundleSetup: { internal: 100, external: 0, label: 'SIP Channel + DID Bundle Activation', unit: 'per bundle/one-time' },
  },
  numbers: {
...
const defaultLineItemSection = (dotPath = '') => {
  ...
  if (category === 'sipDidBundles') return 'SIP Channels + DIDs Bundle';
...
const catLabels = {
  sipTrunk: 'SIP Trunk',
  channels: 'Channels',
  sipDidBundles: 'SIP Channel + DID Bundles',
  numbers: 'Numbers / DID',
...
const catOrder = ['sipTrunk', 'channels', 'sipDidBundles', 'numbers', 'integration', 'cloudPBX', 'professionalServices', 'callRates'];
...
const mergeCostDB = (savedDB = {}) => {
  ...
  Object.keys(DEFAULT_COST_DB).forEach(cat => {
    merged[cat] = { ...DEFAULT_COST_DB[cat], ...(savedDB[cat] || {}) };
  });
  Object.keys(savedDB || {}).forEach(cat => {
    if (!merged[cat]) merged[cat] = savedDB[cat];
  });
  return merged;
};
...
export default function SIPPricingPlaybook() {
  ...
  const [costDB, setCostDB] = useState(() => {
    try {
      ...
      return saved ? mergeCostDB(JSON.parse(saved)) : mergeCostDB(DEFAULT_COST_DB);
    } catch (e) { return mergeCostDB(DEFAULT_COST_DB); }
  });
